...
static const void* _BZFSOperationRetainObjCObject(const void* info) { return [(id)info retain]; }

static void _BZFSOperationReleaseObjCObject(const void* info) { [(id)info release]; }

static CFStringRef _BZFSOperationDescribeObjCObject(const void* info) { return CFStringCreateCopy(NULL, (CFStringRef)[(id)info description]); }

static void _BZFSOperationStatusCallback(FSFileOperationRef fileOp, const char* currentItem, FSFileOperationStage stage, OSStatus error,
...
- (BOOL)allowOverwriting { return (_options & kFSFileOperationOverwrite) ? YES : NO; }
