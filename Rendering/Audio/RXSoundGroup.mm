...
- (NSUInteger)hash { return hash_combine(hash_intptr((uintptr_t)parent), twav_id); }

- (NSString*)description
{
  return [NSString stringWithFormat:@"%@ {parent=%@, ID=%hu, gain=%f, pan=%f, detach_timestamp=%qu, source=%p}", [super description], parent, twav_id, gain,
                                    pan, detach_timestamp, source];
}
...
- (BOOL)isEqual:(id)anObject { return (anObject == self) ? YES : NO; }
