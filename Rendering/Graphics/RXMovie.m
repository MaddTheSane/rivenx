...
- (id)initWithMovie:(Movie)movie disposeWhenDone:(BOOL)disposeWhenDone owner:(id)owner
{
  ...
  if (_current_size.width < 32) {
    ...
    if (err != noErr) {
      ...
      @throw [NSException exceptionWithName:@"RXMovieException"
                                   ...
                                   userInfo:[NSDictionary dictionaryWithObject:[RXError errorWithDomain:NSOSStatusErrorDomain code:err userInfo:nil]
  ...
  glReportError();
  glVertexAttribPointer(RX_ATTRIB_POSITION, 2, GL_FLOAT, GL_FALSE, 0, _coordinates);
  glReportError();

  glEnableVertexAttribArray(RX_ATTRIB_TEXCOORD0);
  glReportError();
  glVertexAttribPointer(RX_ATTRIB_TEXCOORD0, 2, GL_FLOAT, GL_FALSE, 0, _coordinates + 8);
  glReportError();
...
- (id)initWithURL:(NSURL*)movieURL owner:(id)owner
{
  ...
  @try { return [self initWithMovie:aMovie disposeWhenDone:YES owner:owner]; }
  @catch (NSException* e)
  {
    DisposeMovie(aMovie);
    @throw e;
...
#if defined(DEBUG)
    RXOLog2(kRXLoggingGraphics, kRXLoggingLevelDebug, @"used smooth movie looping hack for %@, original duration=%@", self,
            QTStringFromTime(_original_duration));
#if DEBUG > 2
...
- (float)volume { return [_movie volume]; }
...
- (void)play { [self setRate:1.0f]; }

- (void)stop { [_movie stop]; }

- (float)rate { return [_movie rate]; }

- (void)setRate:(float)rate { [_movie setRate:rate]; }
    ...
    if (_image_buffer) {
      if (CFGetTypeID(_image_buffer) == CVOpenGLTextureGetTypeID()) {
        ...
        for (GLint row = 0; row < height; row++)
          memcpy(BUFFER_OFFSET(_texture_storage, (row * MAX((GLint)_current_size.width, 128)) << 2), BUFFER_OFFSET(baseAddress, row * bpr), width << 2);
        ...
        glTexSubImage2D(GL_TEXTURE_RECTANGLE_ARB, 0, 0, 0, MAX(_current_size.width, 128), height, GL_BGRA,
#if defined(__LITTLE_ENDIAN__)
