...
- (id)initWithFrame:(NSRect)frame
{
  ...
  NSDictionary* error_info = [NSDictionary
      dictionaryWithObjectsAndKeys:NSLocalizedStringFromTable(@"NO_SUPPORTED_GPU", @"Rendering", @"no supported gpu"), NSLocalizedDescriptionKey,
                                   NSLocalizedStringFromTable(@"UPGRADE_OS_OR_HARDWARE", @"Rendering", @"upgrade Mac OS X or computer or gpu"),
                                   NSLocalizedRecoverySuggestionErrorKey, [NSArray arrayWithObjects:NSLocalizedString(@"QUIT", @"quit"), nil],
                                   NSLocalizedRecoveryOptionsErrorKey, [RXApplicationDelegate sharedApplicationDelegate], NSRecoveryAttempterErrorKey, nil];
...
#if DEBUG
    RXOLog2(kRXLoggingGraphics, kRXLoggingLevelDebug, @"virtual screen %d is driven by the \"%@\" renderer", ipix, [RXWorldView rendererNameForID:renderer]);
#endif
...
- (CGLContextObj)renderContext { return _renderContextCGL; }

- (CGLContextObj)loadContext { return _loadContextCGL; }

- (CGLPixelFormatObj)cglPixelFormat { return _cglPixelFormat; }

- (CVDisplayLinkRef)displayLink { return _displayLink; }

- (CGColorSpaceRef)workingColorSpace { return _workingColorSpace; }

- (CGColorSpaceRef)displayColorSpace { return _displayColorSpace; }

- (rx_size_t)viewportSize { return RXSizeMake(_glWidth, _glHeight); }

- (void)setCardRenderer:(id)renderer { _cardRenderer = RXGetRenderer(renderer); }

- (NSCursor*)cursor { return _cursor; }
...
- (void)mouseDown:(NSEvent*)event { [[g_world cardRenderer] mouseDown:event]; }

- (void)mouseUp:(NSEvent*)event { [[g_world cardRenderer] mouseUp:event]; }

- (void)mouseMoved:(NSEvent*)event { [[g_world cardRenderer] mouseMoved:event]; }

- (void)mouseDragged:(NSEvent*)event { [[g_world cardRenderer] mouseDragged:event]; }

- (void)swipeWithEvent:(NSEvent*)event { [[g_world cardRenderer] swipeWithEvent:event]; }

- (void)keyDown:(NSEvent*)event { [[g_world cardRenderer] keyDown:event]; }
...
- (void)_initializeCardRendering
{
  ...
  glReportError();
  glFramebufferTexture2DEXT(GL_FRAMEBUFFER_EXT, GL_COLOR_ATTACHMENT0_EXT, GL_TEXTURE_RECTANGLE_ARB, _cardTexture, 0);
  glReportError();
  ...
  // create the card VAO
  glGenVertexArraysAPPLE(1, &_cardVAO);
  glReportError();
  ...
  glReportError();
  glVertexAttribPointer(RX_ATTRIB_POSITION, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), NULL);
  glReportError();

  glEnableVertexAttribArray(RX_ATTRIB_TEXCOORD0);
  glReportError();
  glVertexAttribPointer(RX_ATTRIB_TEXCOORD0, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), BUFFER_OFFSET(NULL, 2 * sizeof(GLfloat)));
  glReportError();

  // create the fade VAO
  glGenVertexArraysAPPLE(1, &_fadeLayerVAO);
  glReportError();
  ...
  glEnableVertexAttribArray(RX_ATTRIB_POSITION);
  glReportError();
  glVertexAttribPointer(RX_ATTRIB_POSITION, 2, GL_FLOAT, GL_FALSE, 4 * sizeof(GLfloat), BUFFER_OFFSET(NULL, 16 * sizeof(GLfloat)));
  glReportError();

  // get a standard card program
  _cardProgram =
      [[GLShaderProgramManager sharedManager] standardProgramWithFragmentShaderName:@"card" extraSources:nil epilogueIndex:0 context:cgl_ctx error:NULL];
  release_assert(_cardProgram);

  glUseProgram(_cardProgram);
  glReportError();

  GLint uniform_loc = glGetUniformLocation(_cardProgram, "destination_card");
  glReportError();
  release_assert(uniform_loc != -1);
  glUniform1i(uniform_loc, 0);
  glReportError();

  uniform_loc = glGetUniformLocation(_cardProgram, "modulate_color");
  glReportError();
  release_assert(uniform_loc != -1);
  glUniform4f(uniform_loc, 1.f, 1.f, 1.f, 1.f);
  glReportError();

  // get the solid color program
  NSDictionary* bindings = [NSDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithInt:RX_ATTRIB_POSITION], @"position", nil];
  ...
  _solidColorLocation = glGetUniformLocation(_solidColorProgram, "color");
  glReportError();
...
- (void)_updateCardCoordinates
{
  ...
  struct _attribs* attribs = (struct _attribs*)glMapBuffer(GL_ARRAY_BUFFER, GL_WRITE_ONLY);
  glReportError();
  ...
  attribs[0].pos[0] = contentRect.origin.x;
  attribs[0].pos[1] = contentRect.origin.y;
  attribs[0].tex[0] = 0.0f;
  attribs[0].tex[1] = 0.0f;

  attribs[1].pos[0] = contentRect.origin.x + contentRect.size.width;
  attribs[1].pos[1] = contentRect.origin.y;
  attribs[1].tex[0] = (GLfloat)kRXCardViewportSize.width;
  attribs[1].tex[1] = 0.0f;

  attribs[2].pos[0] = contentRect.origin.x;
  attribs[2].pos[1] = contentRect.origin.y + contentRect.size.height;
  attribs[2].tex[0] = 0.0f;
  attribs[2].tex[1] = (GLfloat)kRXCardViewportSize.height;

  attribs[3].pos[0] = contentRect.origin.x + contentRect.size.width;
  attribs[3].pos[1] = contentRect.origin.y + contentRect.size.height;
  attribs[3].tex[0] = (GLfloat)kRXCardViewportSize.width;
  attribs[3].tex[1] = (GLfloat)kRXCardViewportSize.height;

  // whole screen
  attribs[4].pos[0] = 0.0f;
  attribs[4].pos[1] = 0.0f;
  attribs[4].tex[0] = 0.0f;
  attribs[4].tex[1] = 0.0f;

  attribs[5].pos[0] = _glWidth;
  attribs[5].pos[1] = 0.0f;
  attribs[5].tex[0] = _glWidth;
  attribs[5].tex[1] = 0.0f;

  attribs[6].pos[0] = 0.0f;
  attribs[6].pos[1] = _glHeight;
  attribs[6].tex[0] = 0.0f;
  attribs[6].tex[1] = _glHeight;

  attribs[7].pos[0] = _glWidth;
  attribs[7].pos[1] = _glHeight;
...
- (void)_determineGLVersion:(CGLContextObj)cgl_ctx
{
  ...
  */
  ...
  if (_glMajorVersion == 1) {
    if ([_gl_extensions containsObject:@"GL_ARB_shader_objects"] && [_gl_extensions containsObject:@"GL_ARB_vertex_shader"] &&
