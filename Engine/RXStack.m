...
- (id)initWithKey:(NSString*)key error:(NSError**)error
{
  ...
  if (!descriptor)
    @throw [NSException exceptionWithName:NSInvalidArgumentException
                                   reason:[NSString stringWithFormat:@"No stack descriptor for stack key %@.", key]
                                 userInfo:nil];
...
- (void)_tearDown
{
  ...
  [_cardNames release];
  _cardNames = nil;
  [_hotspotNames release];
  _hotspotNames = nil;
  [_externalNames release];
  _externalNames = nil;
  [_varNames release];
  _varNames = nil;
  [_stackNames release];
...
- (NSString*)description { return [NSString stringWithFormat:@"%@{%@}", [super description], _key]; }

- (NSString*)debugName { return _key; }

#pragma mark -

- (NSString*)key { return _key; }

- (uint16_t)entryCardID { return _entryCardID; }
...
- (NSString*)cardNameAtIndex:(uint32_t)index { return (_cardNames) ? [_cardNames objectAtIndex:index] : nil; }

- (NSString*)hotspotNameAtIndex:(uint32_t)index { return (_hotspotNames) ? [_hotspotNames objectAtIndex:index] : nil; }

- (NSString*)externalNameAtIndex:(uint32_t)index { return (_externalNames) ? [_externalNames objectAtIndex:index] : nil; }

- (NSString*)varNameAtIndex:(uint32_t)index { return (_varNames) ? [_varNames objectAtIndex:index] : nil; }
...
- (NSString*)stackNameAtIndex:(uint32_t)index { return (_stackNames) ? [_stackNames objectAtIndex:index] : nil; }
