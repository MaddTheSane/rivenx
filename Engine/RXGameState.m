  ...
  @catch (NSException* e)
  {
    if (error) {
      if ([[e userInfo] objectForKey:NSUnderlyingErrorKey])
        ...
        *error = [RXError errorWithDomain:RXErrorDomain code:0 userInfo:[NSDictionary dictionaryWithObject:[e reason] forKey:NSLocalizedDescriptionKey]];
    }
  }
...
- (id)init
{
  ...
  RXSimpleCardDescriptor* scd =
      [[RXSimpleCardDescriptor alloc] initWithStackKey:@"aspit" ID:[[[g_world stackDescriptorForKey:@"aspit"] objectForKey:@"Entry"] unsignedShortValue]];
...
- (void)dump { RXOLog(@"dumping\n%@", _variables); }

- (NSURL*)URL { return _URL; }

- (BOOL)writeToURL:(NSURL*)url error:(NSError**)error { return [self writeToURL:url updateURL:YES error:error]; }
...
- (BOOL)writeToURL:(NSURL*)url updateURL:(BOOL)update error:(NSError**)error
{
  ...
  if (!gameStateData)
    ReturnValueWithError(NO, RXErrorDomain, 0,
                         ([NSDictionary dictionaryWithObject:@"Riven X was unable to prepare the game to be saved." forKey:NSLocalizedDescriptionKey]), error);
